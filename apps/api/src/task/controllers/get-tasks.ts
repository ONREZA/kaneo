...
async function getTasks(projectId: string, options: GetTasksOptions = {}) {
  ...
  if (options.dueBefore) {
    const d = new Date(options.dueBefore);
    if (Number.isNaN(d.getTime())) {
      throw new HTTPException(400, {
        message: "Invalid dueBefore date format",
      });
    }
  }

  if (options.dueAfter) {
    const d = new Date(options.dueAfter);
    if (Number.isNaN(d.getTime())) {
      throw new HTTPException(400, { message: "Invalid dueAfter date format" });
    }
  }
  ...
  const paginatedTasks = hasPagination
    ? await taskQuery.limit(pageSize).offset(offset)
    : await taskQuery;
